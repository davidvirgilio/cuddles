...
import style from "@/ui/components/show-posts/show-posts.module.sass"
...
export default function Like({postId, userId, initialLikes}:{postId: string, userId: any, initialLikes: string[]}){
    ...
    const handleLikeToggle = async () => {
        try {
            const response = await fetch(`/api/mongodb/likes?_id=${postId}&user_id=${userId}`, {
                method: 'PATCH',
            });
            ...
            if (response.ok) {
                ...
                console.error("Failed to update likes", await response.json());
            }
        } catch (error) {
    ...
    return (
        <button onClick={handleLikeToggle} className={`${style.likeButton} ${hasLiked ? 'liked' : ''}`}>
            {hasLiked ? (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="34" height="36" viewBox="0 0 34 36" fill="none" className="like">
