...
export function sortPostsByTime( posts: post[] ){
...
export function Post( { post, isProfile, index} : PostProps ){
    ...
    const username = post.user_id?.username;
    const profilePic = post.user_id?.profile_pic;
        ...
        <div className={style.post}>
            ...
            <div className={style.engagement}>
                <div className={style.icons}>
                    ...
                    <button>
                        <Link href={`/comment/${post._id}`} scroll={false}>
                ...
                <div className={style.commentButtons}>
                    {
                        post.comments.length > 0 && (
                            <Link href={`/comment/${post._id}`} scroll={false}>View all comments</Link>
                        )
                    }
                    <Link href={`/comment/${post._id}`} scroll={false}>+ Add comment</Link>
                </div>
...
export default function Posts({ posts, isProfile = false}: PostsProps ){
    ...
    return(
        <div>
            { sortedPosts.map((post, index)=> <Post key={index} index={index} post = {post} isProfile={ isProfile }/>) }
        </div>
    )
}
